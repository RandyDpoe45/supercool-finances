...
export class TransactionRepository implements ITransactionRepository {
  ...
  async transitionToReversedInTx(
    queryRunner: QueryRunner,
    id: string,
    reason: string = 'rail_settlement_failed',
  ): Promise<boolean> {
    // Guarded UPDATE: the `status = POSTED` predicate is the atomic idempotency gate for a reversal.
    // affected === 1 means THIS call won the reversal (the caller then posts the compensating
    // movement); 0 means the row was already reversed / is not posted, so a retried or concurrent
    // reversal is a no-op — no double reversal. `failure_reason` records WHY the posted movement was
    // reversed — the rail FAILURE callback (5c) keeps the default `'rail_settlement_failed'`, the
    // maker-checker admin reversal (8b) passes `'admin_reversal'`, so the admin transaction view
    // never mislabels an admin-reversed internal transfer as a rail failure. There is no dedicated
    // `reversed_at` column; the compensating transaction (with its own `posted_at` +
    // `reverses_transaction_id`) is the audit record of when/what reversed it.
    const result = await queryRunner.manager
      .createQueryBuilder()
      .update(Transaction)
      .set({ status: TransactionStatus.Reversed, failureReason: reason })
