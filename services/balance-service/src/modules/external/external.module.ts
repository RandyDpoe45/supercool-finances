 ...
 * it is guarded globally by `RailSignatureGuard` (bound in {@link AppModule}), which requires the
 * rail's HMAC `X-Rail-Signature` over the raw body. This module carries no guard of its own (the
 * guard is prefix-scoped + global).
 */
@Module({
  imports: [RailsModule],
