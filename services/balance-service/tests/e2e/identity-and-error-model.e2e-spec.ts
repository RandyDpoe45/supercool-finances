/**
 * DoD (spec 03) + Step-3a coordination contract:
 *   2. Gateway identity guard  (/api, /admin)
 *   3. Service identity guard  (/internal)
 *   4. Error model DTO  (+ requestId correlation)
 *
 ...
 * the gateway guard ignores /internal, the service guard ignores /api — which is the
 * "applied globally per prefix, cannot be skipped" property from the spec.
 *
 * The guards/filter/middleware are the implementor's real code; the probe
 * controllers and test module are scaffolding only. A wrong implementation (trusting
 * a body/query id, forgetting the admin check, wrong status, missing/altered error
 * DTO, dropped correlation id) fails a test here.
 */
...
class ApiProbeController {
  ...
  probe(@Req() req: any) {
    ...
    // test proves the guard parsed + attached the identity.
...
// message embeds a secret-looking DSN, to prove the 5xx genericization path.
const SENSITIVE_LEAK = 'sensitive: postgres://user:pass@host/db';
...
describe('identity guards + error model (minimal app, no Docker)', () => {
  ...
  describe('gateway identity guard (/api)', () => {
    ...
    it('accepts and resolves the identity to the HEADER value, parsing X-Roles into an array', async () => {
    ...
    it('does NOT trust a userId from query or body (no header => still 401)', async () => {
      const res = await request(app.getHttpServer())
        .get('/api/probe?userId=user-mallory')
    ...
    it('uses the HEADER identity, never a conflicting query/body id (BOLA guard)', async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.identity.userId).toBe('user-alice'); // header wins
    });
  });

  describe('admin role enforcement (/admin)', () => {
    it('rejects with 401 when no gateway identity is present at all', async () => {
    ...
    it('allows when the identity carries the admin role', async () => {
      const res = await request(app.getHttpServer())
        .get('/admin/probe')
        .set('X-User-Id', 'user-bob')
        .set('X-Roles', 'customer,admin');
  ...
  describe('guards self-scope by prefix (global, but per-plane)', () => {
    it('the service-token guard does NOT gate /api (a valid user reaches /api with no service token)', async () => {
      const res = await request(app.getHttpServer())
        .get('/api/probe')
        .set('X-User-Id', 'user-alice')
        .set('X-Roles', 'customer');
  ...
  describe('guards fail CLOSED on mixed-case prefixes (regression: case-bypass)', () => {
    // Express routes case-insensitively, so /API, /ADMIN and /INTERNAL reach the same
    ...
    // skip auth simply by upper-casing the path (the fixed HIGH).
    ...
    it('GET /API/probe with no X-User-Id still requires identity (401, not 200)', async () => {
      const res = await request(app.getHttpServer()).get('/API/probe');
      expect(res.status).toBe(401);
    });
...
describe('error filter genericizes 5xx failures (no info leak)', () => {
  ...
  it('maps an unexpected Error to a generic 500 and leaks neither the cause nor a stack', async () => {
    ...
    expect(serialized).not.toContain('postgres://user:pass@host');
