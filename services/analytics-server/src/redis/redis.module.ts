 ...
 * everywhere — the spec 05 stream consumer is the sole reader on the analytics side.
 * Mirrors the balance service's `RedisModule` so both ends share one connection posture.
 *
 * Boot resilience (the non-obvious *why*):
 * - `lazyConnect: true` — no socket is opened until the first command, so booting
 *   AppModule while Redis is down neither connects nor throws (integration/e2e tests
 *   that boot AppModule without Redis, and the disabled-consumer path, must not fail here).
 ...
 *   reconnect (surface a failure via the command promise, not a hard cap). Blocking
 *   reads (the consumer's `XREADGROUP ... BLOCK`) must not be capped out mid-wait.
 */
@Global()
@Module({
  providers: [
