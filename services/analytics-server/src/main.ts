 ...
 *   1. validate config (fail fast)  ->  2. create app (opens the Mongo connection)
 *   ->  3. cross-cutting middleware; global guards + exception filter are bound in
 *   AppModule  ->  4. listen.
 */
async function bootstrap(): Promise<void> {
  ...
  new Logger('Bootstrap').log(`analytics-server listening on port ${config.port}`);
}
