...
| R1 | the demo dataset matches the spec **exactly**: all 11 customers (id/name/phone/email) — `demo-customer` #1, `demo-customer-2 … -10`, and Maria — with the **right number of accounts each** (1, except `demo-customer-2`=3, `-3`=2, `-4`=3, `-5`=2), and every one of the **17** accounts active MXN with its exact account_number, balance, owner, `held`=0, spend counters 0, non-null `spent_*_date` = `CURRENT_DATE`; exactly **11** customers / **17** customer accounts | DoD "Seed data is present". Fails on any wrong/missing field, a missing/extra account, or wrong cardinality. |
