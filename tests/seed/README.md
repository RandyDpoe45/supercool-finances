...
| 3 | **Pinned subs** — `realm-export.json` pins `demo-customer` = `1111…`, `demo-admin` = `2222…`, and `demo-customer`'s email matches Customer A | "Sub alignment": the sub must be deterministic. **FAILs** on a missing/mismatched pinned id — the cheap half of the load-bearing link. |
...
| R1 | the demo dataset matches the spec **exactly**: both customers (id/name/phone/email), each with **one** active MXN account (account_number, balance, `held`=0, spend counters 0, non-null `spent_*_date` = `CURRENT_DATE`), and exactly **2** customers / **2** customer accounts | DoD "Seed data is present". Fails on any wrong/missing field, a missing customer, or wrong cardinality. |
| R3 | **sub alignment** — the seeded `customer.id` **and** `account.owner_id` for account `1000000001` both equal the `demo-customer` pinned sub | DoD "Keycloak logins map to seeded customers". The load-bearing link, proven against the real row. |
