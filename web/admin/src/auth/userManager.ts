...
const CLIENT_ID_DEFAULT = 'admin-app';
...
/**
 * OIDC settings for the admin SPA: Authorization Code + PKCE (S256) against the Keycloak
 * `admin-app` public client. The admin app is root-served on its dedicated internal origin,
 * so `redirect_uri`/`post_logout_redirect_uri` are the app origin, which matches the realm's
 * allowlisted `http://localhost:8081/*` for the `admin-app` client
 * (tools/keycloak/realm-export.json). The `supercool-api` audience is added by a Keycloak
 * mapper, so it is NOT requested as a scope here — only `openid profile`.
 */
 ...
 * one source of truth, so the bearer attached to `/balance/admin` calls always reflects the
 * current session.
 */
