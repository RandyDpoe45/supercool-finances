/**
 * Date/time rendering for the customer SPA. The server is UTC-only and sends ISO-8601
 * `Z` instants; the UI converts to Mexico City time AT THE EDGE (spec 07 / cross-cutting
 * rule 5) — the server never sees a localized time. The IANA zone is applied via `Intl`,
 * NEVER a hardcoded UTC offset, so DST history (and Mexico's 2022 DST abolition) is left
 * to the tz database rather than baked in here.
 */
